...
class ConnectMysql(object):
    ...
    def getTable(self):
        ...
        for table in tables:
            ...
            # 需要迁移的数据库查询表的列数
            cur.execute("SELECT COUNT(*) FROM information_schema.COLUMNS WHERE table_schema='fcdb' AND table_name='" + table[0] + "'")
            ...
            # 阿里云数据库创建表
            ll = "SELECT table_name FROM information_schema.`TABLES` WHERE table_schema='fcdb' AND table_name='" + str(table[0]).lower() + "'"
